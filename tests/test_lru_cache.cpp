...
TEST_CASE("LRUCache: basic put and get") {
    utils2::LRUCache<int, std::string> cache;
    cache.put(1, "one");
    cache.put(2, "two");
...
TEST_CASE("LRUCache: custom size_fn is used") {
    utils2::LRUCache<int, std::string>::Config cfg;
    cfg.max_bytes = 20;
    cfg.size_fn = [](const std::string& s) -> std::size_t { return s.size(); };
    utils2::LRUCache<int, std::string> cache(cfg);
...
TEST_CASE("LRUCache: HasByteSize concept path used when no size_fn") {
    ...
    utils2::LRUCache<int, SizedValue>::Config cfg;
    cfg.max_bytes = 100;
    // Intentionally NOT setting cfg.size_fn
    utils2::LRUCache<int, SizedValue> cache(cfg);
