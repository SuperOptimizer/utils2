...
TEST_CASE("ZarrDtype size") {
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::uint8), std::size_t(1));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::uint16), std::size_t(2));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::uint32), std::size_t(4));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::uint64), std::size_t(8));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::float32), std::size_t(4));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::float64), std::size_t(8));
}

TEST_CASE("ZarrDtype parse_dtype") {
    auto d1 = utils2::parse_dtype("<u2");
    REQUIRE(d1.has_value());
    REQUIRE_EQ(*d1, utils2::ZarrDtype::uint16);

    auto d2 = utils2::parse_dtype(">f4");
    REQUIRE(d2.has_value());
    REQUIRE_EQ(*d2, utils2::ZarrDtype::float32);

    auto d3 = utils2::parse_dtype("|u1");
    REQUIRE(d3.has_value());
    REQUIRE_EQ(*d3, utils2::ZarrDtype::uint8);

    auto d4 = utils2::parse_dtype("i4");
    REQUIRE(d4.has_value());
    REQUIRE_EQ(*d4, utils2::ZarrDtype::int32);

    auto bad = utils2::parse_dtype("xyz");
    REQUIRE(!bad.has_value());
}

TEST_CASE("ZarrDtype dtype_string") {
    REQUIRE_EQ(utils2::dtype_string(utils2::ZarrDtype::uint16), std::string_view("u2"));
    REQUIRE_EQ(utils2::dtype_string(utils2::ZarrDtype::float64), std::string_view("f8"));
}

TEST_CASE("ZarrMetadata helpers") {
    utils2::ZarrMetadata meta;
    meta.shape = {100, 200, 300};
    meta.chunks = {10, 20, 30};
    meta.dtype = utils2::ZarrDtype::uint16;
...
TEST_CASE("ZarrArray create and read back") {
    ...
    auto arr = utils2::ZarrArray::create(dir / "test.zarr", meta);
    ...
    REQUIRE_EQ(arr.metadata().dtype, utils2::ZarrDtype::uint16);

    // Re-open and verify metadata persisted.
    auto arr2 = utils2::ZarrArray::open(dir / "test.zarr");
    REQUIRE_EQ(arr2.metadata().shape[0], std::size_t(64));
    REQUIRE_EQ(arr2.metadata().dtype, utils2::ZarrDtype::uint16);
...
TEST_CASE("ZarrArray chunk write/read round trip") {
    ...
    meta.shape = {16, 16};
    meta.chunks = {8, 8};
    meta.dtype = utils2::ZarrDtype::float32;

    auto arr = utils2::ZarrArray::create(dir / "arr.zarr", meta);
...
TEST_CASE("ZarrArray chunk not found returns nullopt") {
    ...
    meta.shape = {16};
    meta.chunks = {8};
    meta.dtype = utils2::ZarrDtype::uint8;
...
TEST_CASE("Zarr pyramid level counting") {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        meta.chunks = {32, 32};
        meta.dtype = utils2::ZarrDtype::uint16;
        utils2::ZarrArray::create(dir / std::to_string(i), meta);
    }

    REQUIRE_EQ(utils2::count_pyramid_levels(dir), std::size_t(3));

    auto levels = utils2::open_pyramid(dir);
...
TEST_CASE("Zarr .zarray JSON round trip") {
    ...
    meta.shape = {128, 256};
    meta.chunks = {64, 64};
    meta.dtype = utils2::ZarrDtype::float64;
    meta.byte_order = '<';
    ...
    auto json = utils2::detail::serialize_zarray(meta);
    auto parsed = utils2::detail::parse_zarray(json);
    ...
    REQUIRE_EQ(parsed.dtype, utils2::ZarrDtype::float64);
...
TEST_CASE("ZarrDtype dtype_string_v3") {
    REQUIRE_EQ(utils2::dtype_string_v3(utils2::ZarrDtype::uint16), std::string_view("uint16"));
    REQUIRE_EQ(utils2::dtype_string_v3(utils2::ZarrDtype::float32), std::string_view("float32"));
    REQUIRE_EQ(utils2::dtype_string_v3(utils2::ZarrDtype::int64), std::string_view("int64"));
    REQUIRE_EQ(utils2::dtype_string_v3(utils2::ZarrDtype::bool_), std::string_view("bool"));
    REQUIRE_EQ(utils2::dtype_string_v3(utils2::ZarrDtype::complex128), std::string_view("complex128"));
}

TEST_CASE("ZarrDtype parse_dtype_v3") {
    auto d1 = utils2::parse_dtype_v3("uint16");
    REQUIRE(d1.has_value());
    REQUIRE_EQ(*d1, utils2::ZarrDtype::uint16);

    auto d2 = utils2::parse_dtype_v3("float64");
    REQUIRE(d2.has_value());
    REQUIRE_EQ(*d2, utils2::ZarrDtype::float64);

    auto d3 = utils2::parse_dtype_v3("bool");
    REQUIRE(d3.has_value());
    REQUIRE_EQ(*d3, utils2::ZarrDtype::bool_);

    auto bad = utils2::parse_dtype_v3("invalid");
    REQUIRE(!bad.has_value());
}

TEST_CASE("ZarrDtype all sizes") {
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::bool_), std::size_t(1));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::int8), std::size_t(1));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::int16), std::size_t(2));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::int32), std::size_t(4));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::int64), std::size_t(8));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::float16), std::size_t(2));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::complex64), std::size_t(8));
    REQUIRE_EQ(utils2::dtype_size(utils2::ZarrDtype::complex128), std::size_t(16));
}

TEST_CASE("ZarrDtype parse_dtype all v2 types") {
    REQUIRE_EQ(*utils2::parse_dtype("b1"), utils2::ZarrDtype::bool_);
    REQUIRE_EQ(*utils2::parse_dtype("i1"), utils2::ZarrDtype::int8);
    REQUIRE_EQ(*utils2::parse_dtype("i2"), utils2::ZarrDtype::int16);
    REQUIRE_EQ(*utils2::parse_dtype("i8"), utils2::ZarrDtype::int64);
    REQUIRE_EQ(*utils2::parse_dtype("f2"), utils2::ZarrDtype::float16);
    REQUIRE_EQ(*utils2::parse_dtype("c8"), utils2::ZarrDtype::complex64);
    REQUIRE_EQ(*utils2::parse_dtype("c16"), utils2::ZarrDtype::complex128);
}

TEST_CASE("ZarrDtype dtype_string_v2 all types") {
    REQUIRE_EQ(utils2::dtype_string_v2(utils2::ZarrDtype::bool_), std::string_view("b1"));
    REQUIRE_EQ(utils2::dtype_string_v2(utils2::ZarrDtype::uint8), std::string_view("u1"));
    REQUIRE_EQ(utils2::dtype_string_v2(utils2::ZarrDtype::int8), std::string_view("i1"));
    REQUIRE_EQ(utils2::dtype_string_v2(utils2::ZarrDtype::float16), std::string_view("f2"));
    REQUIRE_EQ(utils2::dtype_string_v2(utils2::ZarrDtype::complex64), std::string_view("c8"));
}
...
TEST_CASE("FileSystemStore operations") {
    ...
    utils2::FileSystemStore store(dir);
...
TEST_CASE("OME-NGFF MultiscaleMetadata parse and serialize round trip") {
    ...
    utils2::MultiscaleMetadata meta;
    meta.version = "0.4";
    meta.name = "test_volume";
    meta.axes = {
        {.name = "z", .type = utils2::AxisType::space, .unit = "micrometer"},
        {.name = "y", .type = utils2::AxisType::space, .unit = "micrometer"},
        {.name = "x", .type = utils2::AxisType::space, .unit = "micrometer"},
    };

    utils2::MultiscaleDataset ds0;
    ds0.path = "0";
    ds0.transforms.push_back(utils2::ScaleTransform{.scale = {1.0, 0.5, 0.5}});
    meta.datasets.push_back(ds0);

    utils2::MultiscaleDataset ds1;
    ds1.path = "1";
    ds1.transforms.push_back(utils2::ScaleTransform{.scale = {1.0, 1.0, 1.0}});
    ...
    auto jv = utils2::serialize_ome_metadata(meta);
    auto parsed = utils2::parse_ome_metadata(jv);
...
TEST_CASE("Zarr .zarray round trip with no compressor") {
    ...
    meta.shape = {64};
    meta.chunks = {64};
    meta.dtype = utils2::ZarrDtype::int16;
    meta.compressor_id = "";

    auto json = utils2::detail::serialize_zarray(meta);
    auto parsed = utils2::detail::parse_zarray(json);

    REQUIRE_EQ(parsed.dtype, utils2::ZarrDtype::int16);
}
...
TEST_CASE("Zarr v3 zarr.json round trip basic") {
    utils2::ZarrMetadata meta;
    meta.version = utils2::ZarrVersion::v3;
    ...
    auto json = utils2::detail::serialize_zarr_json(meta);
    auto parsed = utils2::detail::parse_zarr_json(json);

    REQUIRE_EQ(parsed.version, utils2::ZarrVersion::v3);
    ...
    REQUIRE_EQ(parsed.chunks[0], std::size_t(64));
    REQUIRE_EQ(parsed.dtype, utils2::ZarrDtype::float32);
    REQUIRE(parsed.fill_value.has_value());
    REQUIRE_NEAR(*parsed.fill_value, 0.0, 1e-12);
...
TEST_CASE("Zarr v3 zarr.json all dtypes round trip") {
    auto test_dtype = [](utils2::ZarrDtype dt) {
        utils2::ZarrMetadata meta;
        meta.version = utils2::ZarrVersion::v3;
        meta.shape = {16};
        meta.chunks = {16};
    ...
    test_dtype(utils2::ZarrDtype::bool_);
    test_dtype(utils2::ZarrDtype::uint8);
    test_dtype(utils2::ZarrDtype::uint16);
    test_dtype(utils2::ZarrDtype::uint32);
    test_dtype(utils2::ZarrDtype::uint64);
    test_dtype(utils2::ZarrDtype::int8);
    test_dtype(utils2::ZarrDtype::int16);
    test_dtype(utils2::ZarrDtype::int32);
    test_dtype(utils2::ZarrDtype::int64);
    test_dtype(utils2::ZarrDtype::float16);
    test_dtype(utils2::ZarrDtype::float32);
    test_dtype(utils2::ZarrDtype::float64);
    test_dtype(utils2::ZarrDtype::complex64);
    test_dtype(utils2::ZarrDtype::complex128);
}
...
TEST_CASE("Zarr v3 codec pipeline parsing") {
    ...
    auto meta = utils2::detail::parse_zarr_json(json_str);
...
TEST_CASE("Zarr v3 codecs serialize round trip") {
    ...
    // Add explicit codecs
    utils2::ZarrCodecConfig bytes_codec;
    bytes_codec.name = "bytes";
    utils2::JsonObject bytes_cfg;
    bytes_cfg["endian"] = utils2::JsonValue("little");
    bytes_codec.configuration = utils2::JsonValue(std::move(bytes_cfg));
    meta.codecs.push_back(bytes_codec);

    utils2::ZarrCodecConfig zstd_codec;
    zstd_codec.name = "zstd";
    utils2::JsonObject zstd_cfg;
    zstd_cfg["level"] = utils2::JsonValue(3);
    zstd_codec.configuration = utils2::JsonValue(std::move(zstd_cfg));
...
TEST_CASE("ShardIndex serialize and deserialize round trip") {
    utils2::detail::ShardIndex index;
    ...
    auto deserialized = utils2::detail::ShardIndex::deserialize(bytes, 4);
...
TEST_CASE("ShardIndexEntry is_missing") {
    utils2::detail::ShardIndexEntry present = {0, 100};
    REQUIRE(!present.is_missing());

    utils2::detail::ShardIndexEntry missing = {~std::uint64_t(0), ~std::uint64_t(0)};
    REQUIRE(missing.is_missing());

    // Default-constructed should be missing
    utils2::detail::ShardIndexEntry default_entry;
...
TEST_CASE("ShardIndex too small throws") {
    ...
    bool threw = false;
    try {
        utils2::detail::ShardIndex::deserialize(too_small, 4);
...
TEST_CASE("LE64 read/write round trip") {
    std::byte buf[8]{};
    utils2::detail::write_le64(buf, 0x0102030405060708ULL);
    auto val = utils2::detail::read_le64(buf);
    REQUIRE_EQ(val, std::uint64_t(0x0102030405060708ULL));

    // Test with zero
    utils2::detail::write_le64(buf, 0);
    REQUIRE_EQ(utils2::detail::read_le64(buf), std::uint64_t(0));

    // Test with max
    utils2::detail::write_le64(buf, ~std::uint64_t(0));
    REQUIRE_EQ(utils2::detail::read_le64(buf), ~std::uint64_t(0));
}
...
TEST_CASE("ZarrMetadata shard helpers") {
    ...
    utils2::ZarrMetadata meta2;
    meta2.shape = {32};
    meta2.chunks = {32};
    meta2.dtype = utils2::ZarrDtype::float32;
...
TEST_CASE("ZarrArray v3 create and read back") {
    ...
    auto arr = utils2::ZarrArray::create(dir / "test_v3.zarr", meta);

    REQUIRE_EQ(arr.version(), utils2::ZarrVersion::v3);
    ...
    // Re-open and verify
    auto arr2 = utils2::ZarrArray::open(dir / "test_v3.zarr");
    REQUIRE_EQ(arr2.version(), utils2::ZarrVersion::v3);
    REQUIRE_EQ(arr2.metadata().dtype, utils2::ZarrDtype::uint16);
...
TEST_CASE("ZarrArray v3 chunk key format") {
    ...
    // v2 encoding
    utils2::ZarrMetadata meta2;
    meta2.version = utils2::ZarrVersion::v3;
    meta2.shape = {32, 32};
    meta2.chunks = {16, 16};
    meta2.dtype = utils2::ZarrDtype::uint8;
    meta2.chunk_key_encoding = "v2";
    meta2.dimension_separator = ".";

    auto arr2 = utils2::ZarrArray::create(dir / "arr2.zarr", meta2);
...
TEST_CASE("ZarrArray v3 sharded write_shard and read_inner_chunk") {
    ...
    auto arr = utils2::ZarrArray::create(dir / "sharded.zarr", meta);
    REQUIRE(arr.is_sharded());
...
TEST_CASE("ZarrArray open from FileSystemStore") {
    ...
    utils2::ZarrArray::create(dir / "myarray", meta);

    // Now open through a Store
    auto store = std::make_shared<utils2::FileSystemStore>(dir);
    auto arr = utils2::ZarrArray::open(store, "myarray");

    REQUIRE_EQ(arr.metadata().shape[0], std::size_t(32));
    REQUIRE_EQ(arr.metadata().dtype, utils2::ZarrDtype::float32);
...
TEST_CASE("ZarrArray open from Store v3") {
    ...
    meta.shape = {64};
    meta.chunks = {32};
    meta.dtype = utils2::ZarrDtype::int32;

    utils2::ZarrArray::create(dir / "v3arr", meta);

    auto store = std::make_shared<utils2::FileSystemStore>(dir);
    auto arr = utils2::ZarrArray::open(store, "v3arr");

    REQUIRE_EQ(arr.version(), utils2::ZarrVersion::v3);
    REQUIRE_EQ(arr.metadata().dtype, utils2::ZarrDtype::int32);
...
TEST_CASE("ZarrFilter delta encode/decode round trip") {
    utils2::ZarrFilter f;
    f.id = utils2::ZarrFilterId::delta;
    f.dtype = utils2::ZarrDtype::int32;
...
TEST_CASE("ZarrFilter fixedscaleoffset encode/decode round trip float64") {
    utils2::ZarrFilter f;
    f.id = utils2::ZarrFilterId::fixedscaleoffset;
    f.dtype = utils2::ZarrDtype::float64;
    f.offset = 100.0;
    f.scale = 2.0;
...
TEST_CASE("ZarrFilter fixedscaleoffset float32") {
    utils2::ZarrFilter f;
    f.id = utils2::ZarrFilterId::fixedscaleoffset;
    f.dtype = utils2::ZarrDtype::float32;
    f.offset = 10.0;
    f.scale = 3.0;
...
TEST_CASE("ZarrFilter fixedscaleoffset non-float passthrough") {
    utils2::ZarrFilter f;
    f.id = utils2::ZarrFilterId::fixedscaleoffset;
    f.dtype = utils2::ZarrDtype::uint16;  // not float -> passthrough
...
TEST_CASE("ZarrFilter quantize encode float64") {
    utils2::ZarrFilter f;
    f.id = utils2::ZarrFilterId::quantize;
    f.dtype = utils2::ZarrDtype::float64;
    f.digits = 2;
...
TEST_CASE("Zarr .zarray round trip with delta filter") {
    ...
    f.id = utils2::ZarrFilterId::delta;
    f.dtype = utils2::ZarrDtype::int32;
    f.astype = utils2::ZarrDtype::int32;
    ...
    REQUIRE_EQ(parsed.filters[0].id, utils2::ZarrFilterId::delta);
    REQUIRE_EQ(parsed.filters[0].dtype, utils2::ZarrDtype::int32);
    REQUIRE_EQ(parsed.filters[0].astype, utils2::ZarrDtype::int32);
}
...
TEST_CASE("Zarr .zarray round trip with fixedscaleoffset filter") {
    ...
    REQUIRE_EQ(parsed.filters.size(), std::size_t(1));
    REQUIRE_EQ(parsed.filters[0].id, utils2::ZarrFilterId::fixedscaleoffset);
...
TEST_CASE("Zarr .zarray round trip with quantize filter") {
    ...
    REQUIRE_EQ(parsed.filters.size(), std::size_t(1));
    REQUIRE_EQ(parsed.filters[0].id, utils2::ZarrFilterId::quantize);
...
TEST_CASE("ZarrArray v2 with delta filter write/read round trip") {
    ...
    auto arr = utils2::ZarrArray::create(dir / "filtered.zarr", meta);
...
TEST_CASE("OmeZarrWriter creates pyramid and OmeZarrReader reads it") {
...

    {
        utils2::MultiscaleDataset ds0;
        ds0.path = "0";
        ds0.transforms.push_back(utils2::ScaleTransform{{1.0, 1.0}});
        ms.datasets.push_back(ds0);
    }
    {
        utils2::MultiscaleDataset ds1;
        ds1.path = "1";
        ds1.transforms.push_back(utils2::ScaleTransform{{2.0, 2.0}});
        ms.datasets.push_back(ds1);
    }

    utils2::OmeZarrWriter::Config cfg;
    cfg.root = dir / "volume.ome.zarr";
    cfg.multiscale = ms;
    cfg.dtype = utils2::ZarrDtype::uint16;
    cfg.chunk_shape = {32, 32};

    utils2::OmeZarrWriter writer(cfg);
    ...
    utils2::OmeZarrReader reader(dir / "volume.ome.zarr");
...
TEST_CASE("OmeZarrWriter too many levels throws") {
    ...
    auto ms = utils2::make_standard_multiscale(
        "small", {16, 16}, 1,
        {
            {.name = "y", .type = utils2::AxisType::space},
            {.name = "x", .type = utils2::AxisType::space},
        });

    utils2::OmeZarrWriter::Config cfg;
    cfg.root = dir / "vol.zarr";
    cfg.multiscale = ms;
    cfg.dtype = utils2::ZarrDtype::uint8;
...
TEST_CASE("LabelMetadata parse round trip") {
    ...
    auto json = utils2::json_object({
        {"image-label", utils2::json_object({
            {"version", utils2::JsonValue("0.4")},
            {"colors", utils2::JsonValue(utils2::JsonArray{
                utils2::json_object({
                    {"label-value", utils2::JsonValue(1)},
                    {"rgba", utils2::JsonValue(utils2::JsonArray{
                        utils2::JsonValue(255), utils2::JsonValue(0),
                        utils2::JsonValue(0), utils2::JsonValue(255)
                    })}
                }),
                utils2::json_object({
                    {"label-value", utils2::JsonValue(2)},
                    {"rgba", utils2::JsonValue(utils2::JsonArray{
                        utils2::JsonValue(0), utils2::JsonValue(255),
                        utils2::JsonValue(0), utils2::JsonValue(128)
                    })}
                })
            })},
            {"properties", utils2::JsonValue(utils2::JsonArray{
                utils2::JsonValue("area"), utils2::JsonValue("centroid")
            })}
        })}
    });

    auto meta = utils2::parse_label_metadata(json);
...
TEST_CASE("PlateMetadata parse round trip") {
    auto json = utils2::json_object({
        {"plate", utils2::json_object({
            {"version", utils2::JsonValue("0.4")},
            {"name", utils2::JsonValue("my_plate")},
            {"field_count", utils2::JsonValue(2)},
            {"columns", utils2::JsonValue(utils2::JsonArray{
                utils2::json_object({{"name", utils2::JsonValue("1")}}),
                utils2::json_object({{"name", utils2::JsonValue("2")}}),
                utils2::json_object({{"name", utils2::JsonValue("3")}})
            })},
            {"rows", utils2::JsonValue(utils2::JsonArray{
                utils2::json_object({{"name", utils2::JsonValue("A")}}),
                utils2::json_object({{"name", utils2::JsonValue("B")}})
            })},
            {"wells", utils2::JsonValue(utils2::JsonArray{
                utils2::json_object({
                    {"path", utils2::JsonValue("A/1")},
                    {"rowIndex", utils2::JsonValue(0)},
                    {"columnIndex", utils2::JsonValue(0)}
                }),
                utils2::json_object({
                    {"path", utils2::JsonValue("B/2")},
                    {"rowIndex", utils2::JsonValue(1)},
                    {"columnIndex", utils2::JsonValue(1)}
                })
            })}
        })}
    });

    auto meta = utils2::parse_plate_metadata(json);
...
TEST_CASE("PlateMetadata parse missing plate throws") {
    auto json = utils2::json_object({{"not_plate", utils2::JsonValue("x")}});
    bool threw = false;
    try {
        (void)utils2::parse_plate_metadata(json);
    } catch (const std::runtime_error&) {
        threw = true;
    }
    REQUIRE(threw);
...
TEST_CASE("CoordinateTransform parse scale and translation") {
    auto json = utils2::JsonValue(utils2::JsonArray{
        utils2::json_object({
            {"type", utils2::JsonValue("scale")},
            {"scale", utils2::JsonValue(utils2::JsonArray{
                utils2::JsonValue(1.0), utils2::JsonValue(2.0), utils2::JsonValue(3.0)
            })}
        }),
        utils2::json_object({
            {"type", utils2::JsonValue("translation")},
            {"translation", utils2::JsonValue(utils2::JsonArray{
                utils2::JsonValue(10.0), utils2::JsonValue(20.0), utils2::JsonValue(30.0)
            })}
        })
    });

    auto transforms = utils2::ome_detail::parse_transforms(json);
    REQUIRE_EQ(transforms.size(), std::size_t(2));

    auto* st = std::get_if<utils2::ScaleTransform>(&transforms[0]);
    REQUIRE(st != nullptr);
    ...
    auto* tt = std::get_if<utils2::TranslationTransform>(&transforms[1]);
    REQUIRE(tt != nullptr);
...
TEST_CASE("CoordinateTransform serialize round trip") {
    std::vector<utils2::CoordinateTransform> transforms;
    transforms.emplace_back(utils2::ScaleTransform{{1.5, 2.5, 3.5}});
    transforms.emplace_back(utils2::TranslationTransform{{100.0, 200.0, 300.0}});

    auto json = utils2::ome_detail::serialize_transforms(transforms);
    auto parsed = utils2::ome_detail::parse_transforms(json);

    REQUIRE_EQ(parsed.size(), std::size_t(2));

    auto* st = std::get_if<utils2::ScaleTransform>(&parsed[0]);
    REQUIRE(st != nullptr);
    REQUIRE_NEAR(st->scale[0], 1.5, 1e-12);

    auto* tt = std::get_if<utils2::TranslationTransform>(&parsed[1]);
    REQUIRE(tt != nullptr);
    REQUIRE_NEAR(tt->translation[0], 100.0, 1e-12);
}

TEST_CASE("CoordinateTransform parse non-array returns empty") {
    auto json = utils2::JsonValue("not an array");
    auto transforms = utils2::ome_detail::parse_transforms(json);
...
TEST_CASE("AxisType parse and string round trip") {
    REQUIRE_EQ(utils2::ome_detail::parse_axis_type("space"), utils2::AxisType::space);
    REQUIRE_EQ(utils2::ome_detail::parse_axis_type("time"), utils2::AxisType::time);
    REQUIRE_EQ(utils2::ome_detail::parse_axis_type("channel"), utils2::AxisType::channel);
    REQUIRE_EQ(utils2::ome_detail::parse_axis_type("unknown"), utils2::AxisType::custom);

    REQUIRE_EQ(utils2::ome_detail::axis_type_string(utils2::AxisType::space), std::string_view("space"));
    REQUIRE_EQ(utils2::ome_detail::axis_type_string(utils2::AxisType::time), std::string_view("time"));
    REQUIRE_EQ(utils2::ome_detail::axis_type_string(utils2::AxisType::channel), std::string_view("channel"));
    REQUIRE_EQ(utils2::ome_detail::axis_type_string(utils2::AxisType::custom), std::string_view("custom"));
}
...
TEST_CASE("make_standard_multiscale") {
    auto ms = utils2::make_standard_multiscale(
        ...
        4,
        {
            {.name = "z", .type = utils2::AxisType::space, .unit = "um"},
            {.name = "y", .type = utils2::AxisType::space, .unit = "um"},
            {.name = "x", .type = utils2::AxisType::space, .unit = "um"},
        },
        {0.5, 0.5, 1.0}
    );
...
TEST_CASE("compute_downsample_factors") {
    auto ms = utils2::make_standard_multiscale(
        "test", {64, 64, 64}, 3,
        {
            {.name = "z", .type = utils2::AxisType::space},
            {.name = "y", .type = utils2::AxisType::space},
            {.name = "x", .type = utils2::AxisType::space},
        },
        {1.0, 1.0, 1.0}
    ...
    auto factors = utils2::compute_downsample_factors(ms, 0, 1);
    ...
    auto factors02 = utils2::compute_downsample_factors(ms, 0, 2);
...
TEST_CASE("compute_downsample_factors out of range throws") {
    ...
    bool threw = false;
    try {
        (void)utils2::compute_downsample_factors(ms, 0, 5);
    } catch (const std::runtime_error&) {
        threw = true;
    }
    REQUIRE(threw);
...
TEST_CASE("MultiscaleMetadata with translation transform round trip") {
    ...
    utils2::MultiscaleDataset ds;
    ds.path = "0";
    ds.transforms.push_back(utils2::ScaleTransform{{1.0, 1.0, 1.0}});
    ds.transforms.push_back(utils2::TranslationTransform{{10.0, 20.0, 30.0}});
    ...
    auto* st = std::get_if<utils2::ScaleTransform>(&parsed.datasets[0].transforms[0]);
    REQUIRE(st != nullptr);

    auto* tt = std::get_if<utils2::TranslationTransform>(&parsed.datasets[0].transforms[1]);
    REQUIRE(tt != nullptr);
    REQUIRE_NEAR(tt->translation[0], 10.0, 1e-12);
    REQUIRE_NEAR(tt->translation[1], 20.0, 1e-12);
    REQUIRE_NEAR(tt->translation[2], 30.0, 1e-12);
...
TEST_CASE("MultiscaleMetadata with time and channel axes") {
    ...
    meta.version = "0.4";
    meta.axes = {
        {.name = "t", .type = utils2::AxisType::time, .unit = "second"},
        {.name = "c", .type = utils2::AxisType::channel},
        {.name = "z", .type = utils2::AxisType::space, .unit = "um"},
        {.name = "y", .type = utils2::AxisType::space, .unit = "um"},
        {.name = "x", .type = utils2::AxisType::space, .unit = "um"},
    };
    ...
    utils2::MultiscaleDataset ds;
    ds.path = "0";
    ds.transforms.push_back(utils2::ScaleTransform{{1.0, 1.0, 0.5, 0.5, 0.5}});
    meta.datasets.push_back(ds);
    ...
    REQUIRE_EQ(parsed.axes[0].type, utils2::AxisType::time);
    REQUIRE_EQ(parsed.axes[0].unit, std::string("second"));
    REQUIRE_EQ(parsed.axes[1].type, utils2::AxisType::channel);
    REQUIRE_EQ(parsed.axes[2].type, utils2::AxisType::space);
}
...
TEST_CASE("MultiscaleMetadata voxel_size out of range throws") {
    utils2::MultiscaleMetadata meta;
    meta.axes = {{.name = "x", .type = utils2::AxisType::space}};
    meta.datasets.push_back(utils2::MultiscaleDataset{.path = "0"});
...
TEST_CASE("ConsolidatedMetadata parse") {
    ...
    auto cm = utils2::detail::ConsolidatedMetadata::parse(json_str);
    ...
    REQUIRE_EQ(cm.arrays["0"].dtype, utils2::ZarrDtype::uint16);
    REQUIRE_EQ(cm.arrays["1"].shape[0], std::size_t(32));
    REQUIRE_EQ(cm.arrays["1"].dtype, utils2::ZarrDtype::float32);
...
TEST_CASE("detect_version v2 vs v3") {
    ...
    utils2::detail::write_file(v2_path / ".zarray", "{}");
    REQUIRE_EQ(utils2::detail::detect_version(v2_path), utils2::ZarrVersion::v2);

    // v3: write zarr.json
    auto v3_path = dir / "v3arr";
    fs::create_directories(v3_path);
    utils2::detail::write_file(v3_path / "zarr.json", "{}");
    REQUIRE_EQ(utils2::detail::detect_version(v3_path), utils2::ZarrVersion::v3);
    ...
    bool threw = false;
    try {
        utils2::detail::detect_version(bad_path);
    } catch (const std::runtime_error&) {
        threw = true;
    }
    REQUIRE(threw);
...
TEST_CASE("byteswap_inplace") {
    ...
    utils2::detail::byteswap_inplace(data, 4);
    ...
    utils2::detail::byteswap_inplace(data2, 1);
    ...
    std::vector<std::byte> data3 = {std::byte(0x01), std::byte(0x02),
                                     std::byte(0x03), std::byte(0x04)};
    utils2::detail::byteswap_inplace(data3, 2);
...
TEST_CASE("detail read_file and write_file round trip") {
    ...
    utils2::detail::write_file(dir / "test.txt", "hello world");
    auto content = utils2::detail::read_file(dir / "test.txt");
    REQUIRE_EQ(content, std::string("hello world"));

    // Bytes version
    std::vector<std::byte> bytes = {std::byte(0xDE), std::byte(0xAD)};
    utils2::detail::write_file_bytes(dir / "test.bin", bytes);
    auto bytes_back = utils2::detail::read_file_bytes(dir / "test.bin");
    ...
    bool threw = false;
    try {
        utils2::detail::read_file(dir / "nonexistent.txt");
    } catch (const std::runtime_error&) {
        threw = true;
    }
    REQUIRE(threw);
...
TEST_CASE("OmeZarrReader labels directory") {
    ...
    utils2::detail::write_file(labels_dir / ".zattrs",
        "{\"labels\": [\"segmentation\"]}\n");

    // Create the label as a full OME-Zarr
    auto label_ms = utils2::make_standard_multiscale(
    ...
    utils2::OmeZarrWriter::Config label_cfg;
    label_cfg.root = labels_dir / "segmentation";
    label_cfg.multiscale = label_ms;
    label_cfg.dtype = utils2::ZarrDtype::uint8;
    label_cfg.chunk_shape = {16, 16};

    utils2::OmeZarrWriter label_writer(label_cfg);
    (void)label_writer.add_level({16, 16});
    label_writer.finalize();

    // Now read
    utils2::OmeZarrReader reader(dir / "img.ome.zarr");
    REQUIRE(reader.has_labels());
...
TEST_CASE("MultiscaleMetadata type field preserved") {
    ...
    meta.datasets.push_back(utils2::MultiscaleDataset{
        .path = "0",
        .transforms = {utils2::ScaleTransform{{1.0, 1.0}}}
    });
...
TEST_CASE("Zarr .zarray with multiple filters round trip") {
    ...
    utils2::ZarrFilter f1;
    f1.id = utils2::ZarrFilterId::delta;
    f1.dtype = utils2::ZarrDtype::float64;
    f1.astype = utils2::ZarrDtype::float64;
    meta.filters.push_back(f1);

    utils2::ZarrFilter f2;
    f2.id = utils2::ZarrFilterId::quantize;
    f2.digits = 5;
    ...
    REQUIRE_EQ(parsed.filters[0].id, utils2::ZarrFilterId::delta);
    REQUIRE_EQ(parsed.filters[1].id, utils2::ZarrFilterId::quantize);
...
TEST_CASE("ZarrArray create and open with CodecRegistry") {
    ...
    utils2::ZarrArray::CodecRegistry registry;
    registry["mycodec"] = utils2::ZarrArray::Codec{
    ...
    auto arr = utils2::ZarrArray::create(dir / "arr.zarr", meta, registry);
    ...
    auto arr2 = utils2::ZarrArray::open(dir / "arr.zarr", registry);
...
TEST_CASE("ZarrArray v3 create with CodecRegistry") {
    ...
    utils2::ZarrCodecConfig custom_codec;
    custom_codec.name = "mycompressor";
    meta.codecs.push_back(custom_codec);

    utils2::ZarrArray::CodecRegistry registry;
    registry["mycompressor"] = utils2::ZarrArray::Codec{
        .compress = [](std::span<const std::byte> data) {
            return std::vector<std::byte>(data.begin(), data.end());
        },
        .decompress = [](std::span<const std::byte> data, std::size_t) {
...
TEST_CASE("is_little_endian") {
    ...
    bool le = utils2::detail::is_little_endian();
