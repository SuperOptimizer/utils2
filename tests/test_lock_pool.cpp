...
TEST_CASE("LockPool: try_lock behavior") {
    ...
    for (int i = 0; i < 100; ++i) {
        if (utils2::LockPool<8>::index(i) != utils2::LockPool<8>::index(10)) {
...
TEST_CASE("LockPool: size is correct") {
    REQUIRE_EQ(utils2::LockPool<16>::size(), 16u);
    REQUIRE_EQ(utils2::LockPool<64>::size(), 64u);
    REQUIRE_EQ(utils2::LockPool<128>::size(), 128u);
}
