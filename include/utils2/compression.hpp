...
#   define UTILS2_HAS_ZSTD 1
#   include <zstd.h>
#else
#   define UTILS2_HAS_ZSTD 0
#endif

#if __has_include(<lz4.h>)
#   define UTILS2_HAS_LZ4 1
#   include <lz4.h>
#else
#   define UTILS2_HAS_LZ4 0
#endif

#if __has_include(<lz4hc.h>)
#   define UTILS2_HAS_LZ4HC 1
#   include <lz4hc.h>
#else
#   define UTILS2_HAS_LZ4HC 0
#endif

#if __has_include(<zlib.h>)
#   define UTILS2_HAS_ZLIB 1
#   include <zlib.h>
#else
#   define UTILS2_HAS_ZLIB 0
#endif

#if __has_include(<blosc.h>)
#   define UTILS2_HAS_BLOSC 1
#   include <blosc.h>
#else
#   define UTILS2_HAS_BLOSC 0
#endif

#if __has_include(<blosc2.h>)
#   define UTILS2_HAS_BLOSC2 1
#   include <blosc2.h>
#else
#   define UTILS2_HAS_BLOSC2 0
#endif
...
[[nodiscard]] constexpr std::string_view codec_name(Codec c) noexcept {
    switch (c) {
        ...
        case Codec::blosc2_lz4:    return "blosc2_lz4";
        case Codec::blosc2_zstd:   return "blosc2_zstd";
    }
    return "unknown";
}
...
namespace detail {
    ...
    inline constexpr CodecEntry codec_table[] = {
        ...
        {"blosc2_lz4",    Codec::blosc2_lz4},
        {"blosc2_zstd",   Codec::blosc2_zstd},
    };
} // namespace detail
...
struct CompressParams {
    ...
    std::size_t num_threads = 1;    // For blosc2 multi-threaded
};
...
namespace detail::blosc2_backend {

inline const char* blosc2_compressor_name(Codec c) {
    switch (c) {
        case Codec::blosc2_lz4:  return "lz4";
        case Codec::blosc2_zstd: return "zstd";
        default: return "lz4";
    }
}
...
inline std::vector<std::byte> do_compress(std::span<const std::byte> in,
    ...
    cparams.compcode  = blosc2_compname_to_compcode(blosc2_compressor_name(p.codec));
    cparams.clevel    = p.level;
    cparams.typesize  = static_cast<int32_t>(p.typesize);
    cparams.blocksize = static_cast<int32_t>(p.blocksize);
    cparams.nthreads  = static_cast<int16_t>(p.num_threads);
    if (p.shuffle == 1) cparams.filters[BLOSC2_MAX_FILTERS - 1] = BLOSC_SHUFFLE;
    else if (p.shuffle == 2) cparams.filters[BLOSC2_MAX_FILTERS - 1] = BLOSC_BITSHUFFLE;
    else cparams.filters[BLOSC2_MAX_FILTERS - 1] = BLOSC_NOSHUFFLE;

    auto bound = in.size() + BLOSC2_MAX_OVERHEAD;
    std::vector<std::byte> out(bound);
    blosc2_context* ctx = blosc2_create_cctx(cparams);
    if (!ctx)
        throw std::runtime_error("blosc2 compress: failed to create context");
    auto rc = blosc2_compress_ctx(
        ctx,
        in.data(), static_cast<int32_t>(in.size()),
        out.data(), static_cast<int32_t>(out.size()));
    blosc2_free_ctx(ctx);
    if (rc <= 0)
        throw std::runtime_error("blosc2 compress: failed with code " + std::to_string(rc));
    out.resize(static_cast<std::size_t>(rc));
    return out;
}
...
inline std::vector<std::byte> do_decompress(std::span<const std::byte> in,
                                            std::size_t expected) {
    if (expected == 0) {
        blosc2_cbuffer_sizes(in.data(), nullptr, nullptr, nullptr);
        int32_t nbytes{}, cbytes{}, blocksize{};
        blosc2_cbuffer_sizes(in.data(), &nbytes, &cbytes, &blocksize);
        expected = static_cast<std::size_t>(nbytes);
    }
    std::vector<std::byte> out(expected);
    ...
    if (!ctx)
        throw std::runtime_error("blosc2 decompress: failed to create context");
    auto rc = blosc2_decompress_ctx(
        ctx,
        in.data(), static_cast<int32_t>(in.size()),
        out.data(), static_cast<int32_t>(out.size()));
    blosc2_free_ctx(ctx);
    if (rc < 0)
        throw std::runtime_error("blosc2 decompress: failed with code " + std::to_string(rc));
    out.resize(static_cast<std::size_t>(rc));
    return out;
}
...
#if UTILS2_HAS_BLOSC2
    register_codec(Codec::blosc2_lz4, CodecImpl{
        blosc2_backend::do_compress, blosc2_backend::do_decompress});
    register_codec(Codec::blosc2_zstd, CodecImpl{
        blosc2_backend::do_compress, blosc2_backend::do_decompress});
#endif
