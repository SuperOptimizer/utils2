...
struct CompositeParams {
    ...
    // Beer-Lambert params
    float extinction = 1.0f;   // absorption coefficient
    float emission   = 1.0f;   // emission coefficient
    float ambient    = 0.1f;   // ambient light factor
...
/// Each layer value is mapped to an alpha via lerp between alpha_min and
/// alpha_max, then blended front-to-back.
[[nodiscard]] constexpr float composite_alpha(
    std::span<const float> layers,
    float alpha_min, float alpha_max) noexcept
{
    if (layers.empty()) return 0.0f;
    ...
    for (float density : layers) {
        // Map density to alpha in [0, 1]
        float a = (range != 0.0f)
            ? saturate((density - alpha_min) / range)
            : (density >= alpha_max ? 1.0f : 0.0f);

        colour        += a * density * transmittance;
